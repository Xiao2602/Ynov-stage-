...
export default function DashboardLayout() {
  ...
  const [isDocumentGenerationMenuOpen, setIsDocumentGenerationMenuOpen] = useState(false);
  ...
  const isAbsenceDashboard = ['/dashboard', '/absences', '/documents/dashboard'].includes(location.pathname);
  ...
  return (
    <div className="app-container">
        ...
        <nav className="sidebar-nav">
          ...
          <div className="sidebar-group">
            ...
            <div className="sidebar-group-items">
              ...
              {canAccess(absenceRoles.myAbsences, role) && <NavLink to="/absences/mes-absences" className={({ isActive }) => `nav-subitem ${isActive ? 'active' : ''}`}>
                <div className="nav-icon"><IconCalendar /></div><span>{role === 'parent' ? parentAbsenceLabel : 'Mes Absences'}</span>
              </NavLink>}
            ...
            <div className="sidebar-group-items">
              {role === 'teacher' ? (
                ...
                <>
                  ...
                  {role === 'admin' && (
                    <div className={`nav-generation-menu ${isDocumentGenerationMenuOpen ? 'open' : ''}`}>
                      <button
                        type="button"
                        className={`nav-subitem nav-menu-toggle ${location.pathname === '/documents/generate' ? 'active' : ''}`}
                        onClick={() => setIsDocumentGenerationMenuOpen((open) => !open)}
                        aria-expanded={isDocumentGenerationMenuOpen}
                      >
                        <div className="nav-icon"><IconDocument /></div>
                        <span>Générer un document</span>
                        <IconChevronDown className="nav-menu-chevron" />
                      </button>
                      {isDocumentGenerationMenuOpen && (
                        <div className="nav-generation-options" aria-label="Types de document à générer">
                          <NavLink to="/documents/generate?type=attestation_reussite" onClick={() => setIsDocumentGenerationMenuOpen(false)} className={({ isActive }) => `nav-generation-option ${isActive && location.search.includes('attestation_reussite') ? 'active' : ''}`}>Attestation de réussite</NavLink>
                          <NavLink to="/documents/generate?type=attestation_sous_reserve" onClick={() => setIsDocumentGenerationMenuOpen(false)} className={({ isActive }) => `nav-generation-option ${isActive && location.search.includes('attestation_sous_reserve') ? 'active' : ''}`}>Attestation sous réserve</NavLink>
                          <NavLink to="/documents/generate?type=certificat_scolarite" onClick={() => setIsDocumentGenerationMenuOpen(false)} className={({ isActive }) => `nav-generation-option ${isActive && location.search.includes('certificat_scolarite') ? 'active' : ''}`}>Certificat de scolarité</NavLink>
                        </div>
                      )}
                    </div>
                  )}
                  {role === 'admin' && <NavLink to="/documents/generated" className={({ isActive }) => `nav-subitem ${isActive ? 'active' : ''}`}>
                    <div className="nav-icon"><IconDocument /></div><span>Documents générés</span>
                  </NavLink>}
                </>
              )}
            </div>
          ...
          <div className="sidebar-group">
            ...
            <div className="sidebar-group-items">
              ...
              {role === 'admin' && (
                <NavLink to="/activity-logs" className={({ isActive }) => `nav-subitem ${isActive ? 'active' : ''}`}>
                  <div className="nav-icon"><IconActivity /></div><span>Activity Logs</span>
                </NavLink>
              )}
              {role === 'admin' && (
        ...
        <div className="sidebar-footer" ref={profileMenuRef}>
          {isProfileMenuOpen && (
            <div className="profile-dropdown-menu">
              ...
              <button className="profile-dropdown-item" onClick={() => { setIsProfileMenuOpen(false); navigate('/profile'); }}>
      ...
      <main className={`main-content ${isAbsenceDashboard ? 'absence-dashboard-main' : ''}`}>
        {!isAbsenceDashboard && <header style={{
          padding: '16px 32px',
        ...
        <div style={{ padding: isAbsenceDashboard ? '38px 48px' : '24px 32px' }}>
