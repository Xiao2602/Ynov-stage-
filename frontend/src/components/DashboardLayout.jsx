...
export default function DashboardLayout() {
  ...
  // Le rôle RH est volontairement limité au tableau de bord.
  // Les droits de traitement des absences appartiennent à l'administrateur et au personnel.
  const staffRoles = ['admin', 'employee', 'super_admin'];
  const absenceRoles = {
    dashboard: ['admin', 'rh', 'student', 'teacher', 'parent', ...staffRoles],
    myAbsences: ['admin', 'employee', 'super_admin', 'student', 'teacher', 'parent'],
    requests: ['admin', 'employee', 'super_admin', 'student', 'teacher'],
  };
  const documentRoles = {
    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {role !== 'rh' && <div className="sidebar-group">
            ...
            <div className="sidebar-group-items">
              {role === 'teacher' ? (
                ...
                <>
                  ...
                  {(role === 'admin' || role === 'employee' || role === 'super_admin') && <NavLink to="/documents/generate?type=attestation_reussite" className={({ isActive }) => `nav-subitem ${isActive ? 'active' : ''}`}>
                    <div className="nav-icon"><IconDocument /></div><span>Générer un document</span>
                  </NavLink>}
                  {(role === 'admin' || role === 'employee' || role === 'super_admin') && <NavLink to="/documents/generated" className={({ isActive }) => `nav-subitem ${isActive ? 'active' : ''}`}>
          ...
          {role !== 'rh' && <div className="sidebar-group">
            ...
            <div className="sidebar-group-items">
              {(role === 'admin' || role === 'super_admin') && <NavLink to="/users" className={({ isActive }) => `nav-subitem ${isActive ? 'active' : ''}`}>
