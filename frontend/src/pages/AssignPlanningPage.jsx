  ...
  for (const [, groupCourses] of Object.entries(groups)) {
    ...
    for (let i = 0; i < groupCourses.length - 1; i++) {
      ...
      if (start2 < end1) {
        ...
        return {
          valid: false,
          error: `Conflit d'horaire : Le cours "${c1.title || 'Sans titre'}" (${c1.start}) et "${c2.title || 'Sans titre'}" (${c2.start}) sont programmés le ${dateLabel} avec seulement ${diffHours}h d'intervalle. Un intervalle minimum de ${MIN_INTERVAL_HOURS}h est obligatoire entre chaque séance pour ce professeur.`
        };
      }
    }
  }
