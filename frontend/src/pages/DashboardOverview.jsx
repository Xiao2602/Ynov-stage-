...
export default function DashboardOverview() {
  ...
  const adminRoles = ['admin', 'administrateur', 'personnel', 'employee'];
  ...
  // Le RH est volontairement limité à cette page.
  if (role === 'rh') {
    return (
      <div className="dashboard-page-content" style={{ padding: '2rem' }}>
        <div className="overview-header">
          <h2 className="overview-title">Tableau de bord</h2>
          <p className="overview-subtitle">Bienvenue dans votre espace Ressources humaines.</p>
        </div>
      </div>
    );
  }

  // --- Rôle Admin / Personnel (affichage des statistiques) ---
  if (adminRoles.includes(role)) {
    if (loading) {
      return (
