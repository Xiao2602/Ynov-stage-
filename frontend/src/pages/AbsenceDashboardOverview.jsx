...
const GLOBAL_STAT_ROLES = ['admin', 'employee', 'super_admin'];
const ACADEMIC_CLASSES = [
  'Bachelor 1', 'Bachelor 2',
  'Bachelor 3 IA', 'Bachelor 3 Génie logiciel', 'Bachelor 3 Cybersécurité',
  'Master 1 IA', 'Master 1 Génie logiciel', 'Master 1 Cybersécurité',
  'Master 2 IA', 'Master 2 Génie logiciel', 'Master 2 Cybersécurité',
];
...
function normalizeClass(value = '') {
  return String(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
    .replace(/bachelor\s*([123])/g, 'b$1').replace(/master\s*([12])/g, 'm$1')
    .replace(/intelligence\s+artificielle/g, 'ia').replace(/genie\s+logiciel/g, 'gl')
    .replace(/cybersecurite/g, 'cyber').replace(/[^a-z0-9]+/g, ' ').trim();
}

function assignedStudentClass(user = {}) {
  return user.className || user.assignedClass || '';
}
...
export default function AbsenceDashboardOverview() {
  ...
  const [dashboard, setDashboard] = useState({ absences: [], stats: null, children: [], users: [], teachers: [], courses: [] });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [selectedWeek, setSelectedWeek] = useState(() => getWeekInputValue(new Date()));
  const [selectedChildId, setSelectedChildId] = useState('');
  const [filters, setFilters] = useState({ className: '', studentId: '', module: '', teacher: '' });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('');
      try {
        const next = { absences: [], stats: null, children: [], users: [], teachers: [], courses: [] };
        if (isGlobal) {
          const [statsResult, absencesResult, usersResult, catalogResult] = await Promise.all([apiFetch('/absences/statistics'), apiFetch('/absences'), apiFetch('/users'), apiFetch('/plannings/catalog')]);
          ...
          next.users = usersResult.success ? usersResult.data || usersResult.users || [] : [];
          next.teachers = catalogResult.success ? catalogResult.teachers || [] : [];
          next.courses = catalogResult.success ? catalogResult.courses || [] : [];
        } else if (role === 'parent') {
          ...
          const [absenceResult, catalogResult] = await Promise.all([apiFetch('/absences/by-course'), apiFetch('/plannings/catalog')]);
          if (!absenceResult.success) throw new Error(absenceResult.error);
          next.absences = absenceResult.absences || [];
          next.courses = catalogResult.success ? catalogResult.courses || [] : [];
  ...
  const filterOptions = useMemo(() => {
    const source = dashboard.absences || [];
    const studentUsers = (dashboard.users || []).filter((item) => item.role === 'student').map((item) => ({
      ...item,
      // Ne jamais utiliser department : une classe doit être explicitement assignée.
      assignedClass: assignedStudentClass(item) || '__unassigned__',
    }));
    const selectedTeacher = (dashboard.teachers || []).find((teacher) => teacher.displayName === filters.teacher);
    const classMap = new Map();
    (dashboard.courses || []).forEach((course) => {
      if (course.group && !classMap.has(normalizeClass(course.group))) classMap.set(normalizeClass(course.group), course.group);
    });
    const classes = [...classMap.values()].sort((a, b) => a.localeCompare(b, 'fr'));
    const inClass = filters.className ? source.filter((item) => normalizeClass(item.className || item.assignedClass) === normalizeClass(filters.className)) : source;
    const students = [...new Map([...inClass.map((item) => [item.userId || item.uid, item.displayName || item.userEmail || 'Élève']), ...studentUsers.filter((item) => !filters.className || normalizeClass(item.assignedClass || item.className || item.department) === normalizeClass(filters.className)).map((item) => [item.uid || item.id, item.displayName || item.email || 'Élève'])]).entries()].filter(([id]) => id);
    const plannedModules = (dashboard.courses || []).filter((course) => (!filters.className || normalizeClass(course.group) === normalizeClass(filters.className)) && (!selectedTeacher || course.teacherUid === selectedTeacher.uid)).map((course) => course.title);
    const modules = [...new Set((selectedTeacher ? plannedModules : [...inClass.map((item) => item.courseName || item.module || item.subject), ...plannedModules]).filter(Boolean))].sort();
    const teachers = (dashboard.teachers || []).filter((teacher) => !filters.className || (dashboard.courses || []).some((course) => course.teacherUid === teacher.uid && normalizeClass(course.group) === normalizeClass(filters.className))).map((teacher) => teacher.displayName).filter(Boolean).sort((a, b) => a.localeCompare(b, 'fr'));
    return { classes, students, modules, teachers };
  }, [dashboard.absences, dashboard.users, dashboard.teachers, dashboard.courses, filters.className, filters.teacher]);

  const filteredAbsences = useMemo(() => displayedAbsences.filter((absence) => {
    if (isGlobal && filters.className && normalizeClass(absence.className || absence.assignedClass) !== normalizeClass(filters.className)) return false;
    if (isGlobal && filters.studentId && (absence.userId || absence.uid) !== filters.studentId) return false;
    if ((isGlobal || role === 'teacher') && filters.module && (absence.courseName || absence.module || absence.subject) !== filters.module) return false;
    if (isGlobal && filters.teacher && (absence.teacherName || absence.declaredByName || absence.teacher) !== filters.teacher) return false;
    return true;
  }), [displayedAbsences, filters, isGlobal, role]);

  const weekRange = useMemo(() => getWeekRange(selectedWeek), [selectedWeek]);
  const weeklyAbsences = useMemo(
    () => filteredAbsences.filter((absence) => isInWeek(absence, weekRange)),
    [filteredAbsences, weekRange],
  );
  ...
  return <section className="absence-dashboard">
    ...
    {loading ? <div className="absence-dashboard-feedback">Chargement du tableau de bord…</div> : error ? <div className="absence-dashboard-feedback is-error"><strong>Impossible de charger les données.</strong><span>{error}</span></div> : <>
      ...
      {(isGlobal || role === 'teacher') && <div className="absence-child-selector absence-dashboard-filters"><label>Filtrer</label><select value={filters.className} onChange={(event) => setFilters({ className: event.target.value, studentId: '', module: '', teacher: '' })}><option value="">Toutes les classes</option>{filterOptions.classes.map((value) => <option key={value} value={value}>{value}</option>)}</select>{isGlobal && filters.className && <select value={filters.studentId} onChange={(event) => setFilters((current) => ({ ...current, studentId: event.target.value }))}><option value="">Tous les élèves</option>{filterOptions.students.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select>}{(filters.className || filters.teacher) && <select value={filters.module} onChange={(event) => setFilters((current) => ({ ...current, module: event.target.value }))}><option value="">Tous les modules</option>{filterOptions.modules.map((value) => <option key={value} value={value}>{value}</option>)}</select>}{isGlobal && <select value={filters.teacher} onChange={(event) => setFilters((current) => ({ ...current, teacher: event.target.value }))}><option value="">Tous les professeurs</option>{filterOptions.teachers.map((value) => <option key={value} value={value}>{value}</option>)}</select>}</div>}
