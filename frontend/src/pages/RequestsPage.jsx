...
export default function RequestsPage() {
  ...
  // Le personnel dispose ici des mêmes droits de traitement que l'administrateur.
  // Le rôle stocké par le backend pour « Personnel » est `employee`.
  const isAdmin = ['admin', 'employee', 'super_admin', 'administrateur', 'personnel'].includes(role);
    ...
    try {
      ...
      if (data && data.success) {
        ...
        const formatted = absences.map(a => ({
          ...
          declaredByName: a.declaredByName || null,
          courseName: a.courseName || null,
          originalReason: a.originalReason || a.reason || '',
          studentJustification: a.studentJustification || '',
          isLate: a.isLate === true || a.type === 'late'
        }));
  ...
  const handleExport = async (format) => {
    if (!isAdmin) {
      alert('Seuls les administrateurs et le personnel peuvent exporter.');
      return;
    }
    try {
  ...
  return (
    <div className="dashboard-scroll-area absence-requests" style={{ height: '100%', overflowY: 'auto' }}>
      {/* EN-TÊTE */}
      ...
      {detailModalRequest && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(3px)' }}>
          <div className="absence-review-modal" style={{ background: '#fff', borderRadius: '12px', width: '100%', maxWidth: '560px', padding: '28px 24px', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ padding: '14px', borderRadius: '10px', background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                <strong>{detailModalRequest.declaredByName ? 'Déclaration pédagogique' : 'Demande déclarée par l’utilisateur'}</strong>
                <div style={{ marginTop: 8 }}>Source : {detailModalRequest.declaredByName ? `Professeur ${detailModalRequest.declaredByName}` : detailModalRequest.displayName}</div>
                {detailModalRequest.courseName && <div style={{ marginTop: 5 }}>Cours : {detailModalRequest.courseName}</div>}
                {detailModalRequest.declaredByName && <div style={{ marginTop: 5 }}>Motif initial : {detailModalRequest.originalReason}</div>}
              </div>
              <div><strong>Objet :</strong> {detailModalRequest.title}</div>
              <div><strong>Type :</strong> {detailModalRequest.isLate ? 'Retard' : detailModalRequest.type}</div>
              ...
              {detailModalRequest.studentJustification && <div style={{ padding: '14px', borderRadius: '10px', background: '#eff6ff', border: '1px solid #bfdbfe' }}><strong>Justification ajoutée par l’étudiant</strong><p style={{ marginTop: 7 }}>{detailModalRequest.studentJustification}</p></div>}
