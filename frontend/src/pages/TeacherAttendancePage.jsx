...
export default function TeacherAttendancePage() {
  ...
  const weeklyCourses = useMemo(() => (planning?.courses || []).filter((course) => {
    if (!course.date) return weekOffset === 0;
    const date = new Date(`${String(course.date).slice(0, 10)}T00:00:00`);
    return !Number.isNaN(date.getTime()) && date >= weekStart && date < weekEnd;
  }), [planning, weekOffset, weekStart, weekEnd]);
  ...
  return (
    <section className="teacher-page">
      ...
      {!loading && !error && (
        <>
          <div className="teacher-attendance-week-nav"><button type="button" onClick={() => setWeekOffset((value) => value - 1)}>← Semaine précédente</button><strong>{weekStart.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })} — {new Date(weekEnd - 86400000).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</strong><button type="button" onClick={() => setWeekOffset(0)}>Aujourd’hui</button><button type="button" onClick={() => setWeekOffset((value) => value + 1)}>Semaine suivante →</button></div>
          <div className="teacher-course-picker" aria-label="Sélectionner un cours de la semaine">
