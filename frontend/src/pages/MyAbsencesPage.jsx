...
export default function MyAbsencesPage() {
  ...
  const fetchAbsences = async () => {
    setLoading(true);
    try {
      const [data, childrenResult] = await Promise.all([
        apiFetch(isParent ? '/absences/children' : '/absences/my'),
        isParent ? apiFetch('/users/my-children') : Promise.resolve(null),
      ]);
      ...
      if (data && data.success) {
        ...
        if (isParent && childrenResult?.success) {
          const nextChildren = childrenResult.children || [];
          setChildren(nextChildren);
          setSelectedChildId((current) => {
            const childIds = nextChildren.map((child) => child.uid || child.id).filter(Boolean);
            return childIds.includes(current) ? current : childIds[0] || '';
          });
        }
      } else {
  ...
  const displayedAbsences = isParent
    ? absences.filter((absence) => (absence.userId || absence.uid || absence.studentUid) === selectedChildId)
    : absences;
  const selectedChild = children.find((child) => (child.uid || child.id) === selectedChildId) || null;
  const normalAbsences = displayedAbsences.filter(a => !isLateAbsence(a));
  const lateAbsences = displayedAbsences.filter(a => isLateAbsence(a));
  ...
  const toJustifyCount = displayedAbsences.filter(a => a.status === 'to_justify' || a.status === 'pending').length;

  // Taux d'assiduité
  const justifiedTotal = displayedAbsences.filter(a => a.status === 'approved').length;
  const totalAbsences = displayedAbsences.length;
  ...
  const filteredAbsences = displayedAbsences.filter(a => {
  ...
  return (
    <div className="dashboard-scroll-area" style={{ height: '100%', overflowY: 'auto', padding: '0 2rem 2rem' }}>
      ...
      <div className="overview-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 className="overview-title">{isParent ? 'Absences de mes enfants' : 'Mes Absences'}</h2>
          <p className="overview-subtitle">{isParent ? selectedChild ? `Vous consultez les absences de ${selectedChild.displayName || selectedChild.name || selectedChild.email || 'votre enfant'}.` : 'Sélectionnez un enfant pour consulter ses absences.' : 'Consultez votre historique et déposez vos justificatifs.'}</p>
        </div>
        {isParent && <label style={{ display: 'grid', gap: '6px', color: '#475569', fontSize: '.8rem', fontWeight: 600 }}>Enfant suivi
          ...
          <table className="data-table" style={{ width: '100%' }}>
            ...
            <tbody>
              {filteredAbsences.map((item) => {
                ...
                const canJustify = !isParent && item.status === 'to_justify' && (!item.justificationDeadline || new Date(item.justificationDeadline) > new Date());
