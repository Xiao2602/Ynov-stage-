...
const GLOBAL_DOCUMENT_ROLES = ['admin', 'employee'];
...
export default function DocumentDashboardOverview() {
  ...
  const filteredDocuments = useMemo(() => categoryFilter ? dashboard.documents.filter((document) => document.category === categoryFilter) : dashboard.documents, [dashboard.documents, categoryFilter]);
  const categories = useMemo(() => [...new Set(dashboard.documents.map((document) => document.category).filter(Boolean))].sort(), [dashboard.documents]);
  const weekRange = useMemo(() => getWeekRange(selectedWeek), [selectedWeek]);
  const weeklyDocuments = useMemo(() => filteredDocuments.filter((document) => isInWeek(document, weekRange)), [filteredDocuments, weekRange]);
  ...
  const documentTrend = useMemo(() => Array.from({ length: 5 }, (_, index) => {
    ...
    return { label: date.toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric' }).replace('.', ''), count: filteredDocuments.filter((document) => { const createdAt = getDate(document.createdAt || document.updatedAt); return createdAt && createdAt >= date && createdAt < nextDate; }).length };
  }), [filteredDocuments, weekRange]);
