...
// On utilise un dossier "uploads" à la racine du backend
 ...
 * @param {string} [category="justificatif"] - Catégorie du document
 */
export async function uploadDocumentService(file, userId, category = "justificatif") {
  try {
    ...
    console.log(`✅ Fichier sauvegardé : ${localFilePath}`);

    // 2. Génération de l'URL d'accès HTTP publique
    ...
    const documentMetadata = {
      ...
      storagePath: relativePath,  // ← chemin relatif, pas absolu
      url: publicUrl,             // ← URL publique !
      archived: false,
    ...
    console.log(`✅ Métadonnées enregistrées dans Firestore (ID: ${documentId})`);
    ...
    console.error("❌ Erreur dans uploadDocumentService:", error);
