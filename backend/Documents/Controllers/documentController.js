...
import { uploadDocumentService, storeGeneratedDocument, createDocumentUrl } from "../Services/documentService.js";
...
export async function handleGenerateAdministrativeDocument(req, res) {
  try {
    ...
    const stored = await storeGeneratedDocument(pdfBuffer, userId, filename);
...
export async function handleViewDocument(req, res) {
  try {
    const result = await getAccessibleDocument(req, req.params.id);
    if (result.error) return res.status(result.status).json({ success: false, error: result.error });
    if (result.document.storageArea === 'supabase') {
      return res.redirect(await createDocumentUrl(result.document.storagePath));
    }
