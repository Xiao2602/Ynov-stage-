...
/**
 * Valide qu'aucun cours pour le même professeur ne se chevauche ou n'a moins de 3h d'intervalle.
 */
export function validatePlanningConflicts(courses) {
  ...
  try {
    ...
    const normalizedCourses = courses
      ...
      .sort((a, b) => {
        ...
        // Vérifier l'intervalle minimal de 3h et l'absence de chevauchement d'horaires
    const conflictValidation = validatePlanningConflicts(normalizedCourses);
    if (!conflictValidation.valid) {
      return { success: false, error: conflictValidation.error };
    }
