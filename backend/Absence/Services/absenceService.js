...
export async function teacherDeclareAbsenceService(teacherUser, { 
  ...
  try {
    ...
    const teacherDoc = await adminDb.collection("users").doc(teacherUser.uid).get();
    const assignedClasses = teacherDoc.exists ? teacherDoc.data().assignedClasses || [] : [];
    const studentClass = String(studentData.className || studentData.department || "").trim().toLowerCase();
    const studentBelongsToTeacher = assignedClasses.some(assignedClass => {
      const normalizedAssignedClass = String(assignedClass || "").trim().toLowerCase();
      return normalizedAssignedClass && studentClass && (
        studentClass === normalizedAssignedClass ||
        studentClass.includes(normalizedAssignedClass) ||
        normalizedAssignedClass.includes(studentClass)
      );
    });

    if (!studentBelongsToTeacher) {
      return { success: false, error: "Vous ne pouvez déclarer une absence que pour vos étudiants." };
    }
