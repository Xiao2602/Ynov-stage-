...
export async function handleAcceptDataTerms(req, res) {
  try {
    ...
    await adminDb.collection("users").doc(req.user.uid).set({
      ...
      dataTermsAcceptedAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    }, { merge: true });

    await logActivity(req.user.uid, 'accept_data_terms', {}, req);

    return res.status(200).json({
      success: true,
      message: "Conditions d'utilisation des données acceptées."
    });
  } catch (error) {
    console.error("Erreur acceptation conditions données :", error);
    return res.status(500).json({
      success: false,
      error: "Impossible d'enregistrer votre acceptation."
    });
  }
}
