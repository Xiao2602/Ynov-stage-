...
export async function requestProfileUpdate(req, res) {
  try {
    ...
    const currentName = userDoc.exists ? (userDoc.data().displayName || userDoc.data().name || userDoc.data().firstName + " " + userDoc.data().lastName) : "Utilisateur";
...
export async function approveRequest(req, res) {
  try {
    ...
    await adminDb.collection("users").doc(uid).update(requestedChanges);
